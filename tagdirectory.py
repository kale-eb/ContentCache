...
def load_memory_log(log_file="memory_log.json"):
...
PID_FILE = "tagdirectory.pid"
LOCK_FILE = "tagdirectory.lock"
...
def initialize_memory_log(log_file="memory_log.json"):
...
def should_retry_failed_file(file_path, max_attempts=3, failed_file="failed_files.json"):
...
def batch_process_files(directory):
    ...
    if memory_summary:
        ...
        print(f"   Memory log saved to: memory_log.json")
    ...
    if stats["errors"]:
        ...
        print(f"\n💡 Failed files are tracked in 'failed_files.json'")
