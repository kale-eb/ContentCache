...
class ElectronBridge:
    ...
    def process_files(self, file_paths: list):
        ...
        # Create stop flag dictionary for directory processing
        ...
        for i, file_path in enumerate(file_paths):
            ...
            try:
                if os.path.isdir(file_path):
                    # Process directory with stop flag
                    self.send_output(f"Processing directory: {file_path}")
                    result = self.service.process_directory(file_path, stop_flag=stop_flag)
    ...
    for line in sys.stdin:
        try:
            ...
            if action == 'process':
                ...
                # Also call the tagdirectory stop function for immediate stopping
                try:
                    ...
                    bridge.send_output("Stop signal sent to processing modules")
                except ImportError:
                    bridge.send_output("Warning: Could not import stop function from tagdirectory")
                except Exception as e:
                    bridge.send_output(f"Warning: Error calling stop function: {e}")
