...
class ContentCacheService:
    ...
    def process_directory(self, directory_path: str, recursive: bool = True, stop_flag: Optional[Dict] = None, **options) -> Dict[str, Any]:
        """
        ...
        Args:
            ...
            stop_flag: Optional stop flag dict with 'stop' key for stopping processing
            **options: Processing options
        ...
        try:
            # Create a progress callback that forwards to our progress callback
            def batch_progress_callback(completed: int, total: int, current_file: str = ""):
                if stop_flag and stop_flag.get('stop', False):
                    return  # Don't emit progress if stopped
                
                # Calculate progress in the range 15-95% to match packaging version
                if total > 0:
                    file_progress = (completed / total) * 80 + 15  # 15% to 95%
                    self._emit_progress("directory_processing", file_progress, 
                                      f"Processing {completed}/{total} files" + 
                                      (f": {os.path.basename(current_file)}" if current_file else ""))
            
            # Use the existing tagdirectory function with enhanced progress callback
            results = batch_process_files(directory_path, progress_callback=batch_progress_callback, stop_flag=stop_flag)
            
            if stop_flag and stop_flag.get('stop', False):
                self._emit_progress("directory_processing", 0, "Directory processing stopped by user")
                return {
                    "directory_path": directory_path,
                    "recursive": recursive,
                    "processed_at": datetime.now().isoformat(),
                    "results": {"stopped": True, "processed": 0, "failed": 0},
                    "success": False,
                    "stopped": True
                }
    ...
    def stop_processing(self) -> Dict[str, Any]:
        """Stop any running processing by calling tagdirectory stop function directly."""
        try:
            from tagdirectory import stop_running_instance
            stop_running_instance()
            return {
                'success': True,
                'message': 'Stop signal sent to tagdirectory processing modules'
            }
        except ImportError as e:
            return {
                'success': False,
                'error': f'Could not import stop function: {e}'
            }
        except Exception as e:
            return {
                'success': False,
                'error': f'Failed to stop processing: {e}'
            }
