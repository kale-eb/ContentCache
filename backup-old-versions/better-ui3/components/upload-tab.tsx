...
export function UploadTab() {
  ...
  useEffect(() => {
    try {
      const persistedLogs = localStorage.getItem('uploadTab_logs')
      const persistedProgress = localStorage.getItem('uploadTab_progress')
      const persistedStatus = localStorage.getItem('uploadTab_status')
      const persistedProcessing = localStorage.getItem('uploadTab_isProcessing')
      ...
      if (persistedProgress) {
        setCurrentProgress(parseFloat(persistedProgress))
      }

      if (persistedStatus) {
        setProcessingStatus(persistedStatus as any)
      }

      if (persistedProcessing) {
        setIsProcessing(persistedProcessing === 'true')
      }
    } catch (error) {
  ...
  useEffect(() => {
    localStorage.setItem('uploadTab_logs', JSON.stringify(logs))
  }, [logs])

  useEffect(() => {
    localStorage.setItem('uploadTab_progress', currentProgress.toString())
  }, [currentProgress])

  useEffect(() => {
    localStorage.setItem('uploadTab_status', processingStatus)
  }, [processingStatus])

  useEffect(() => {
    localStorage.setItem('uploadTab_isProcessing', isProcessing.toString())
  }, [isProcessing])
  ...
  const clearLogs = () => {
    ...
    localStorage.removeItem('uploadTab_logs')
    localStorage.removeItem('uploadTab_progress')
    localStorage.removeItem('uploadTab_status')
    localStorage.removeItem('uploadTab_isProcessing')
  }
